...
public class ApplicationManager {
    ...
    public Application submitApplication(String studentID, String internshipID) {
        ...
        String applicationID = systemManager.generateApplicationID();
        Application application = new Application(applicationID, studentID, internshipID);
        
        if (systemManager.addApplication(application)) {
    ...
    public List<Application> getApplicationsByStudent(String studentID) {
        return systemManager.getApplications().values().stream()
    ...
    public List<Application> getApplicationsByRepresentative(String representativeID) {
        List<String> internshipIDs = systemManager.getInternships().values().stream()
    ...
    public boolean approveApplication(String applicationID, String representativeID) {
        Application application = systemManager.getApplication(applicationID);
        if (application == null || application.getStatus() != ApplicationStatus.PENDING) {
            return false;
        }
        
        InternshipOpportunity internship = systemManager.getInternship(application.getInternshipID());
    ...
    public boolean approveWithdrawal(String applicationID) {
        ...
        User user = systemManager.getUser(application.getStudentID());
        InternshipOpportunity internship = systemManager.getInternship(application.getInternshipID());
    ...
    public Map<String, Integer> getApplicationStatistics() {
        ...
        Collection<Application> applications = systemManager.getApplications().values();
        
        stats.put("Total", applications.size());
        stats.put("Pending", (int) applications.stream().filter(a -> a.getStatus() == ApplicationStatus.PENDING).count());
        stats.put("Successful", (int) applications.stream().filter(a -> a.getStatus() == ApplicationStatus.SUCCESSFUL).count());
        stats.put("Unsuccessful", (int) applications.stream().filter(a -> a.getStatus() == ApplicationStatus.UNSUCCESSFUL).count());
        stats.put("Withdrawn", (int) applications.stream().filter(a -> a.getStatus() == ApplicationStatus.WITHDRAWN).count());
        stats.put("Withdrawal Requests", (int) applications.stream().filter(Application::isWithdrawalRequested).count());
